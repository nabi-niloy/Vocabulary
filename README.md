...
<head>
    ...
    <style>
        ...
        .flashcard-container {
            ...
            min-height: 350px; /* Min-height instead of fixed height */
            margin: 2rem auto;
            position: relative;
        }
        ...
        .flashcard {
            width: 100%;
            height: 100%; /* Will adapt to content due to min-height on container */
            position: absolute;
        ...
        .flashcard-word {
            ...
            align-items: center;
            justify-content: center;
            gap: 0.75rem; /* Space between word and icon */
        }

        .pronounce-btn {
            background: none;
            border: none;
            color: #a78bfa; /* Match word color */
            font-size: 1.8rem;
            cursor: pointer;
...
<body class="bg-gray-100 min-h-screen flex flex-col items-center justify-center relative">
        ...
        <div id="flashcard-section" class="flashcard-container">
            <div id="flashcard" class="flashcard cursor-pointer">
                <div class="flashcard-front">
                    <p class="flashcard-word" id="flashcard-word-front">
                        Select a Letter to Begin!
                        <button class="pronounce-btn" id="pronounce-front-btn" aria-label="Pronounce word">
                            <i class="fas fa-volume-up"></i>
                        </button>
                    </p>
                </div>
                <div class="flashcard-back">
                    <p class="flashcard-word" id="flashcard-word-back">
                        <!-- Word will be inserted here -->
                        <button class="pronounce-btn" id="pronounce-back-btn" aria-label="Pronounce word">
                            <i class="fas fa-volume-up"></i>
                        </button>
                    </p>
    ...
    <script>
        ...
        // This array is now empty. Please add your 601 words here following the format:
        // { word: "yourword", definition: "your definition", examples: ["example sentence 1.", "example sentence 2."] }
        const allWords = [
        ...
        { word: "avoid", definition: "to prevent from happening; stay away from", examples: ["Define ecotourism as travel that aims not only to avoid harming the environment, but also to make a positive contribution to the local ecology and culture.", "We can avoid damaging the environment if we are careful."] },
        ...
        function setHumanLikeVoice() {
            const voices = synth.getVoices();
            // Try to find a specific natural-sounding voice
            // Common natural voices might include "Google US English", "Microsoft Zira - English (United States)", "Samantha" (iOS/macOS)
            let selectedVoice = voices.find(voice => voice.lang === 'en-US' && voice.name.includes('Google US English'));
            if (!selectedVoice) {
                selectedVoice = voices.find(voice => voice.lang === 'en-US' && voice.name.includes('Zira'));
            }
            if (!selectedVoice) {
                selectedVoice = voices.find(voice => voice.lang === 'en-US' && voice.name.includes('Samantha'));
            }
            // Fallback to any en-US voice if specific ones aren't found
            if (!selectedVoice) {
                selectedVoice = voices.find(voice => voice.lang === 'en-US');
            }
            // If still no en-US voice, just use the first available voice
            ...
            if (selectedVoice) {
                ...
                // Optional: Adjust rate and pitch for more natural sound
        ...
        // Listen for voices to be loaded, then set the voice
        synth.onvoiceschanged = setHumanLikeVoice;

        // Call setHumanLikeVoice immediately in case voices are already loaded
        setHumanLikeVoice();
        ...
        function displayFlashcard() {
            if (currentCategoryWords.length === 0) {
                flashcardWordFront.innerHTML = `No words for this letter. <button class="pronounce-btn" id="pronounce-front-btn" aria-label="Pronounce word" disabled><i class="fas fa-volume-up"></i></button>`;
                flashcardWordBack.innerHTML = ``;
            ...
            flashcardWordFront.innerHTML = `${wordObj.word} <button class="pronounce-btn" id="pronounce-front-btn" aria-label="Pronounce word"><i class="fas fa-volume-up"></i></button>`;
            flashcardWordBack.innerHTML = `${wordObj.word} <button class="pronounce-btn" id="pronounce-back-btn" aria-label="Pronounce word"><i class="fas fa-volume-up"></i></button>`;
            ...
            // Re-attach event listeners for pronunciation buttons
            document.getElementById('pronounce-front-btn').onclick = (e) => {
                e.stopPropagation(); // Prevent flashcard flip
                speakWord(wordObj.word);
            };
            document.getElementById('pronounce-back-btn').onclick = (e) => {
                e.stopPropagation(); // Prevent flashcard flip
                speakWord(wordObj.word);
            };
